...
import { useCustomerUserActivities } from "@/hooks/use-customer-user-activities";
...
export function UserActivitySection({ userId }: Props) {
  const t = useTranslations("Users");
  const [page, setPage] = React.useState(1);

  const { data, isLoading, isFetching } = useCustomerUserActivities(userId);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-xs">All recorded activities for this user.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
