...
export function VendorPaymentFilters({ onChange }: Props) {
  const tCommon = useTranslations("Vendor.common");
  const tFilter = useTranslations("Vendor.payments.filters");
  const tStat = useTranslations("Vendor.payments.stats");
      ...
      <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 md:grid-cols-3 md:items-end">
        ...
        <Select value={status} onValueChange={(v) => v && setStatus(v)}>
          ...
          <SelectContent side="bottom">
            ...
            <SelectItem value="pending_payment">{tStat("pending_payment")}</SelectItem>
            <SelectItem value="partially_paid">{tStat("partially_paid")}</SelectItem>
