...
import { deleteAdminCargoCategory, fetchAdminCargoCategories } from "@/lib/admin-api";
...
import { MasterCargoCategoryDialog } from "@/components/dashboard/admin/master/master-cargo-category-dialog";
...
export default function MasterCargoCategoriesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetchAdminCargoCategories({
        page,
        perPage: PER_PAGE,
      ...
      setError(e instanceof ApiError ? e.message : "Gagal memuat kategori kargo.");
      setRows([]);
      setMeta(null);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAdminCargoCategory(Number(deleteRow.id));
      toast.success("Kategori kargo berhasil dihapus.");
      setDeleteOpen(false);
      setDeleteRow(null);
      await load();
    } catch (e) {
  ...
  return (
    <>
      ...
      <MasterTableShell title="Daftar Kategori Kargo" description="Master kategori barang (General cargo, DG, dll)." loading={loading} toolbar={toolbar}>
        <div className="overflow-x-auto -mx-1 px-1">
          <Table>
            ...
            {rows.length === 0 ? (
              <TableCaption className="text-xs">Tidak ada data kategori kargo.</TableCaption>
            ) : (
      ...
      <ConfirmDeleteDialog
        ...
        description={`Yakin hapus kategori "${String(deleteRow?.name ?? "")}"? Tindakan ini tidak dapat dibatalkan.`}
