...
import { ADMIN_REPORT_PER_PAGE, adminBookingReportExportUrl, fetchAdminBookingReport, fetchAllAdminCompanies } from "@/lib/admin-api";
...
export default function AdminBookingReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetchAdminBookingReport({ page, perPage: PER_PAGE, ...filterParams });
  ...
  return (
    <div className={ADMIN_LIST_PAGE_CLASS}>
      <AdminPageHeader icon={BarChart3} title={t("booking.title")} description={t("booking.subtitle")} actions={
        <AdminReportExportButtons buildUrl={adminBookingReportExportUrl} params={filterParams} />
      } />
          ...
          <>
            <Table>
              <TableHeader><TableRow>
                ...
                <TableHead>{t("booking.columns.bookingNo")}</TableHead>
                <TableHead>{tc("table.customer")}</TableHead>
                <TableHead>{t("booking.columns.route")}</TableHead>
                <TableHead>{t("booking.columns.service")}</TableHead>
              ...
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={String(r.id ?? i)}>
                    ...
                    <TableCell className="font-mono text-xs">{String(r.booking_number ?? "—")}</TableCell>
