...
import { adminCustomerInvoiceReportExportUrl, fetchAdminCompanies, fetchAdminCustomerInvoiceReport } from "@/lib/admin-api";
...
export default function AdminCustomerInvoiceReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetchAdminCustomerInvoiceReport({ page, perPage: PER_PAGE, ...filterParams });
  ...
  return (
    <div className={ADMIN_LIST_PAGE_CLASS}>
      <AdminPageHeader icon={BarChart3} title={t("customerInvoice.title")} description={t("customerInvoice.subtitle")} actions={
        <AdminReportExportButtons buildUrl={adminCustomerInvoiceReportExportUrl} params={filterParams} />
      } />
      <Card><CardHeader><CardTitle>{t("reportData")}</CardTitle></CardHeader><CardContent className="space-y-4">
        <div className="flex flex-wrap gap-3">
          ...
          <Select value={statusFilter} onValueChange={(v) => v && setStatusFilter(v)}>
            ...
            <SelectContent><SelectItem value="all">{tc("filters.allStatus")}</SelectItem><SelectItem value="draft">draft</SelectItem><SelectItem value="issued">issued</SelectItem><SelectItem value="paid">paid</SelectItem></SelectContent>
          </Select>
          ...
          <>
            <Table>
              <TableHeader><TableRow>
                ...
                <TableHead>{t("customerInvoice.columns.invoiceNo")}</TableHead>
                <TableHead>{tc("table.customer")}</TableHead>
                <TableHead>{t("customerInvoice.columns.shipment")}</TableHead>
                <TableHead>{t("customerInvoice.columns.issuedDate")}</TableHead>
                <TableHead className="text-right">{t("customerInvoice.columns.amount")}</TableHead>
